...
- **Cache-busting (v0.0.5+):** both shared assets are linked with a version query —
  `css/styles.css?v=0.0.10` and `js/main.js?v=0.0.10` — on every page. The `?v=` is bumped
...
| `v0.0.10` | _(pending)_ | **Complete Hero Imagery Rollout (100% of 19 pages).** Added dedicated background photography (`hero-home.jpg`, `hero-sectors.jpg`, `hero-power-energy.jpg`, `hero-data-centres.jpg`, `hero-building-services.jpg`, `hero-renewables.jpg`) across `index.html`, `sectors.html`, and all 12 job detail pages. Homepage `.hero.has-bg` blends background photography behind the live `#embers` canvas and SVG streak overlay. All 19 pages on the site now feature custom hero background imagery. Homepage hero needed a new `.hero.has-bg` rule (the existing `.page-hero.has-bg` did not apply to `.hero`); its `::before` overrides `.hero::before` at higher specificity, neutralising that rule's size/offset/filter so the darkening overlay covers the photo while `.hero::after`'s glow survives. Fixed a sector mismatch: `job-electrical-maintenance-engineer.html` had a power-energy hero despite being a Building Services role (its eyebrow, `data-sector` and `job-figure` all say Building Services). Cache-busted to `?v=0.0.10`. |
| `v0.0.9` | `909ccb8` | Job detail page hero fix: the back-link and the sector eyebrow rendered on the **same line** ("← All Opportunities — RENEWABLES & ENERGY STORAGE"). Both were `display: inline-flex`, so the two inline-level boxes shared a line and the back-link's `margin-bottom` was silently swallowed. Fixed by making `.back-link` `display: flex` with `width: fit-content`. `.eyebrow` was left alone deliberately: it is used in 40+ places site-wide. Cache-busted to `?v=0.0.9`. || `v0.0.8` | `11c73dd` | **Full site photography rollout (IMAGE-BRIEF.md complete).** 14 master images generated and processed into `images/` & `images/sectors/`. Renewables sector source, wide, tile, and composited branded OG card produced via Pillow (`renewables-og.jpg`); page heroes added to `about.html`, `clients.html`, `candidates.html`, `contact.html`, and `jobs.html`; section imagery added to client & candidate journey steps; split panels added to `index.html`. Generic Open Graph fallback updated to `og-default.jpg` across core pages. Asset query parameters bumped to `?v=0.0.8` across all 19 HTML files. |
